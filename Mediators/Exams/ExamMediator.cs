...
namespace ExaminationSystem.Mediators.Exams
{
    public class ExamMediator : IExamMediator
    {
        ...
        public async Task<bool> TakeExam(ExamStudentCreateDTO examStudentDTO)
        {
            ...
            if (examStudent != null)
            {
                throw new Exception("Student has already started this exam");
            }
    
            // Create new exam student record
            await _examStudentService.AddExamStudent(examStudentDTO);
            return true;
        }
        ...
        public async Task<bool> SubmitExam(ExamStudentDTO examStudentDTO)
        {
            ...
            if (DateTime.Now > exam.DueDate)
            {
                throw new Exception("Exam submission time has expired.");
            }

            var examStudent = await _examStudentService.GetExamStudent(new ExamStudentCreateDTO
            {
                ExamId = examStudentDTO.ExamId,
                StudentId = examStudentDTO.StudentId
            });

            if (examStudent == null)
            {
                throw new Exception("Student hasn't started this exam");
            }

            examStudentDTO.Id = examStudent.Id;  // Set the Id from the existing record
            var result = await _examStudentService.SubmitExam(examStudentDTO);
            return result != null;
        }
    }
}
